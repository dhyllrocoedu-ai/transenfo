                ...
                <form method="POST" action="{{ route('zones.update', $zone) }}">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <select name="team_id" class="form-select">
                                ...
                                @foreach ($teams as $team)
                                    <option value="{{ $team->id }}" @selected(old('team_id', $zone->team_id) == $team->id)>{{ $team->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.0000001" name="center_latitude" id="zone-lat" class="form-control @error('center_latitude') is-invalid @enderror" value="{{ old('center_latitude', $zone->center_latitude) }}" required>
                            @error('center_latitude')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Center Longitude</label>
                            <input type="number" step="0.0000001" name="center_longitude" id="zone-lng" class="form-control @error('center_longitude') is-invalid @enderror" value="{{ old('center_longitude', $zone->center_longitude) }}" required>
                            @error('center_longitude')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Radius (meters)</label>
                            <input type="number" step="1" name="radius_m" id="zone-radius" class="form-control @error('radius_m') is-invalid @enderror" value="{{ old('radius_m', $zone->radius_m) }}" required>
                    ...
                    <div class="d-flex gap-2 mt-4 pt-3 border-top">
                        <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg"></i> Update Zone</button>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body p-3">
                ...
                <div class="form-text mt-2 text-center">
                    <i class="bi bi-info-circle"></i> Click on the map to move the zone center. Adjust the radius in the form.
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (typeof fn === 'function') {
        fn('zone-editor-map', {
            initialLat: {{ $zone->center_latitude }},
            initialLng: {{ $zone->center_longitude }},
            initialRadius: {{ $zone->radius_m }},
            latInputId: 'zone-lat',
            lngInputId: 'zone-lng',
